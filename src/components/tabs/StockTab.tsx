...
import { fmtNum, stockProfitOf, stockProfitPercentOf, fmtStockPercent, signColor, percentColor, badgeStyle } from '../../utils'
              ...
              <div key={t.id} className="expense-card" onClick={() => onEdit(t)}>
                <div className="expense-card-row1">
                  <div className="expense-badges">
                    <span className="category-badge" style={badgeStyle(color)}>{cat?.label ?? t.category}</span>
                    <span className="method-badge" style={badgeStyle('#94A3B8')}>{t.quantity}주</span>
                  </div>
