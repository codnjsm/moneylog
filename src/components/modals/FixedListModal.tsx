...
interface Props {
  items: FixedItem[]
  onSave: (items: Array<{ id: string; label: string; amount: number; paymentDay?: number }>) => void | Promise<void>
  onClose: () => void
}
            ...
            <div key={r.id} className="savings-card">
              ...
              <div className="form-row" style={{ marginTop: 8 }}>
                <div className="form-group">
                  <label htmlFor={`fixed-${r.id}-day`}>납입일</label>
                  <input id={`fixed-${r.id}-day`} type="number" value={r.paymentDay} onChange={e => update(r.id, 'paymentDay', e.target.value)} placeholder="매월 N일" min={1} max={31} />
                </div>
              </div>
            </div>
          ))}
